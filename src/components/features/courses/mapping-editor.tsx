...
//
// Two controls, and only one of them acts:
//
//   Save mapping — `updateCoursePolicy`, which accepts a mapping only while the course has no
//                  confirmed run. When one exists the service answers MAPPING_CHANGE_UNCONFIRMED
//                  and that envelope message is what the person reads; this form does not guess at
//                  the confirmed count, because the count is the service's to know.
//   Apply        — inert. Re-pointing the points of runs that are already confirmed means previewing
//                  the affected runs and re-exporting each of them (FR-206), which is Phase 11's
//                  work. The control stays visible and keeps its accessible name so the sentence
//                  beneath it can say when it arrives, and `aria-disabled` keeps it reachable by
//                  keyboard so a screen reader meets that sentence too.
...
export function MappingEditor({ courseId, mapping, readOnly = false }: MappingEditorProps) {
  ...
  const fieldId = useId()
  const applyNoteId = `${fieldId}-apply-note`
        ...
        {readOnly ? (
          ...
          <div className="flex flex-col gap-2">
            <div className="flex flex-wrap items-center gap-2">
              ...
              >
                {t('courses.mappingApply')}
              </Button>
            </div>
            <p id={applyNoteId} className="text-ink-muted text-body max-w-[60ch]">
