...
## Stack
TypeScript strict · pnpm 11 · Node 24 · Next.js 16 App Router (Node runtime only, `proxy.ts`, Server Actions) · Tailwind 4 + shadcn/ui themed by `DESIGN.md` · react-hook-form + Zod 4 · Drizzle + postgres-js on Postgres 17 (Docker locally, Neon in preview/production) · Better Auth (email+password, Google, organizations) · Resend + react-email · pg-boss jobs · Vercel AI SDK 7 behind `LlmProvider` (`mock` default; Claude Opus 5 via `@ai-sdk/anthropic` in production, D-749; MiMo-V2.5-Pro via OpenAI-compatible API) · Sentry + pino · PostHog · Vitest, Testing Library, Playwright + axe, MSW, Lighthouse CI · ESLint (boundaries) + Prettier · GitHub Actions → Vercel CLI.
