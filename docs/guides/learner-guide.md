...
A Student does not see **Courses**, **Review**, **Packages** or **Admin**; those rail items belong to the other three roles.
...
- **Student** — the role that takes a run; the product's word for you. Every account holds one of four roles: an **Instructor** reads your run and confirms its bands; a **Scenario Editor** builds and publishes scenario packages, and can also take runs; a **Platform Admin** runs the installation and sets every account's role.
...
Verified by automated tests: 2026-09-11, commit 5db6f4b
