...
**The live model.** On the production deployment the assistant, the band reads and generation run on Claude Opus 5 from Anthropic (`claude-opus-5`). Its prose differs from run to run; the claim cards do not, because the scenario's authored trigger phrases decide which claims surface. A reply takes a few seconds. If the model declines a request, the student still gets the claims that request surfaced, with the sentence **The assistant could not add commentary on this request.**, and the run carries on: a declined request is not an outage and never pauses a run.
