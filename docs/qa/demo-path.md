...
3. **Sections** → **New section** → Section name → **Add section** → **Roster** on the section's row → **Add member** → Email address, Role in this section → **Add to section**.
...
5. Student: **Runs** → **Start** on the Decision Run 1 (walkthrough) row → "Before you begin": run type, weight, outside-AI policy, band-to-points table, working clock, "The Readiness Check comes first" → **Begin the Readiness Check**.
...
7. "The scenario": read **Scenario brief**; in **Evidence Room** press **Open** on two or three documents and **Close** on each (opens are recorded). The **AI assistant** panel is locked until the frame is locked.
...
10. On each claim card, under **Your stance**: Accept / Verify / Challenge / Reject / Escalate. **Check it** on Claim C5 → **Source Trace 1 min** → dialog shows Document, Passage, Date, Author. Change the stance afterwards → "Changed from …". **Reject** on C8. **Escalate** on Claim C7 → "What you cannot settle" → **Send it** → "They answered".
11. **Delegation Log**: **Why you asked** under delegation 1 → **Save**; **Mark as used** beside Claim C3. **Declare outside-tool use** → What you used, and what for → **Record it** → "Recorded. It sits with the run and changes nothing about it."
12. (Forced failure, PRD step 7.) Instructor: **Courses** → **Marketing Strategy Walkthrough** → **Assignments** → **Decision Run 1 (walkthrough)** → in **Runs**, **Open the replay** on Student One's row → **Actions** → **Arm the outage**. Student sends any request → dialog "The run is paused" → **Resume the run**; the clock stopped meanwhile.
13. **Your decision brief**: Your recommendation (120), Why (250), three assumptions, What would change your mind (60), Confidence, "Premium payback you are betting on, in months" = 11 → **Lock the decision** → dialog "File this decision?" → **File it** — refused while a relied-on claim has no stance: the dialog becomes "A claim you leaned on has no stance" and names it → **Go to the claim** → take the stance → **Lock the decision** → **File it** → "Decision locked" page: "The Turn" with the countdown to it, the filed decision, the locked frame, **Add an addendum** (50 words, one per run).
...
19. **Actions** tab: **Enter a correction on C3…** → reason, optional "Credit the student's challenge as correct", **Enter the correction** → "What the correction moved" (a correction never lowers a band; export version 2 is written) → **Close**. The auto-lock run's replay is reached the same way (**Auto-lock test run** under **Assignments** → **Open the replay** on Student One's row): **Void this run…** → reason, **Offer the student another run** → **Void the run** → the student's Runs page shows "This attempt was voided." and a new attempt.
...
21. **Runs** → **Open the Judgment Record** on the run's row → **Judgment Record** (`/records/{runId}`): the four graphs, the seven confirmed bands with notes, Mode Standard, Variant Defective, **Download record** ("It carries no course arithmetic, so nothing in it is a grade.").

### E. The auto-lock branch (PRD step 8)

22. Student: **Runs** → **Start** on the Auto-lock test run row → Readiness Check → lock the frame → do nothing for two minutes → the page moves to "Decision locked" with "Left empty." on every field and "No figure." on the named fields; the trace records the auto-lock. The instructor voids this run in step 19.

### F. The sound variant (PRD step 16), seats swapped

23. Student Two runs "Decision Run 1 (sound)": ask "What is the value tier payback?" → **Claim C1** → stance **Accept** without a check → **Mark as used** beside Claim C1 → lock → the Turn (stance Verify on C2 and C3) → Revise → defense → the instructor confirms the remaining drafts → the confirmed debrief shows C1 "Your stance Accept / Warranted Accept / Same" and "Defects the decision rested on" is "Not drawn for this run"; the record shows Variant Sound.

### G. Standing rules and accessibility (PRD step 17)

24. Instructor: on the **Ownership** card choose **Unassessed** → **Record this dimension as Unassessed** → the points table divides by 6. Every run screen has one h1, a **Skip to main content** link, and every control reachable by Tab; each graph has **Show data table**.
