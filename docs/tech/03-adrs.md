...
**Decision.** Option 1; the adapter is compatible with `drizzle-orm@0.45.2`. Plugins: `organization` (its access control knows the one membership role `member`; the account's role lives in `user.platform_role`, D-748), `nextCookies`. Rate limiting stored in the database. Sessions 30-day rolling.
...
**Consequences.** Only the Platform Admin (`admin`) crosses tenants, through the guards that admit it (D-748); every other role, `tassl_scenario_editor` included, reaches only the institutions it belongs to. No read is gated on a data agreement.
