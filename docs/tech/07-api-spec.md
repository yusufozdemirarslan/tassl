...
Auth column legend below: **S** = any session; **Stu** = run owner (the learner: a Student or Scenario Editor); **Rev** = reviewer of the run's course (an Instructor who created it or is on one of its section rosters); **Ins** = an Instructor of the institution; **CI** = course instructor; **Auth** = a Scenario Editor of the package's institution; **Read** = a Scenario Editor or Instructor of the package's institution; **Adm** = Platform Admin, who is admitted to every row below (D-748); **Cron** = cron secret; **Pub** = public. Rate column names the bucket. Errors column lists codes beyond the global ones (`UNAUTHENTICATED`, `FORBIDDEN`, `NOT_FOUND`, `VALIDATION_ERROR`, `RATE_LIMITED`, `INTERNAL_ERROR`).
...
| Method and path | Purpose | Auth | Request | Response | Errors | Rate | Example |
|---|---|---|---|---|---|---|---|
| `GET /me` | Current user with memberships and capabilities | S | — | `MeView` | — | read | `→ {"id":"…","name":"Student One","email":"student1@tassl.local","platformRole":"student","memberships":[{"organizationId":"…","name":"Walkthrough University"}],"activeOrganizationId":"…"}` |
...
| `PATCH /institutions/{orgId}/settings` | Plan label, default mapping | Adm | `{ plan?, defaultMapping? }` | `InstitutionView` | `MAPPING_INVALID` | write | `{"defaultMapping":{"novice":1,"developing":2,"proficient":3,"professional":4}}` |
| `POST /institutions/{orgId}/invitations` | Invite by email (SYS-005); a membership carries no role | Ins, Adm | `{ email }` | `Invitation` | `INVITATION_EMAIL_MISMATCH` | write | `{"email":"student3@tassl.local"}` |
| `POST /invitations/{invitationId}/accept` | Accept invitation | S | — | `Membership` | `INVITATION_EMAIL_MISMATCH` | write | — |
| `GET /institutions/{orgId}/agreements` | Data agreements (FR-234) | Adm | — | `DataAgreement[]` | — | read | — |
| `POST /institutions/{orgId}/agreements` | Create agreement | Adm | `DataAgreementInput` | `DataAgreement` | `AGREEMENT_PURPOSES_INVALID` | write | `{"counterparty":"Walkthrough University","permittedPlatformRoles":["tassl_scenario_editor"],"purposes":["scoring_audit"],"retentionDays":365,"documentReference":"DA-2026-01","signedAt":"2026-09-01T00:00:00Z"}` |
| `PATCH /agreements/{agreementId}` | Update or end agreement | Adm | partial + `endsAt?` | `DataAgreement` | same | write | — |

## 5. Courses and assignments

| Method and path | Purpose | Auth | Request | Response | Errors | Rate | Example |
|---|---|---|---|---|---|---|---|
| `GET /institutions/{orgId}/courses` | List courses (FR-200) | Ins, Adm | `?cursor&limit` | `Page<CourseSummary>` | — | read | — |
| `POST /institutions/{orgId}/courses` | Create course | Ins, Adm | `CreateCourse` | `Course` | `MAPPING_INVALID` | write | `{"name":"Marketing Strategy Walkthrough","term":"2026-fall","outsideAiPolicy":"declared","defaultRunWeight":2.5}` |
| `GET /courses/{courseId}` | Course detail with sections and assignments | Ins, Adm | — | `CourseView` | — | read | — |
...
| `GET /sections/{sectionId}/members` | Roster (SYS-005) | CI | `?cursor&limit` | `Page<SectionMember>` | — | read | — |
| `POST /sections/{sectionId}/members` | Add org member to section by email; the row carries no role (`SectionMember.role` reads back the person's platform role) | CI | `{ email }` | `SectionMember` | `NOT_SECTION_MEMBER` (email not in org) | write | `{"email":"student1@tassl.local"}` |
...
| `GET /assignments/{assignmentId}` | Assignment with package and variant | Stu on the roster, Rev | — | `AssignmentView` | — | read | — |
...
| `GET /assignments/{assignmentId}/policy-display` | Policy display values (FR-201) | Stu on the roster, Rev | — | `PolicyDisplay` | — | read | `→ {"outsideAiPolicy":"declared","weight":2.5,"mapping":{…},"runType":"decision","workingClockSeconds":1500,"uncalibrated":true,"countsStatement":true}` |
...
| Method and path | Purpose | Auth | Request | Response | Errors | Rate | Example |
|---|---|---|---|---|---|---|---|
| `GET /institutions/{orgId}/packages` | Packages with latest version status and warnings | Read | `?cursor&limit` | `Page<PackageSummary>` | — | read | — |
...
| `GET /package-versions/{versionId}/generation` | Generation status | Auth | — | `GenerationStatus` | — | read | `→ {"steps":[{"step":"documents","status":"succeeded","passNumber":1,"inputTokens":…}],"validation":{"ok":false,"failures":["NO_ACCEPT_WARRANTED_SOUND"]}}` |
...
| `POST /runs/{runId}/lock` | Decision Lock (FR-084, FR-102) | Stu | `Brief` | `RunSummary` (`turnDueAt`) | `LOCK_REFUSED_UNSTANCED_CLAIM` (details `{ claimId, claimText }`), `BRIEF_INVALID`, `RUN_PAUSED` | write | `→ 409 {"error":{"code":"LOCK_REFUSED_UNSTANCED_CLAIM","message":"A claim you leaned on has no stance.","details":{"claimId":"…","claimText":"Premium payback is 11 months."},"requestId":"…"}}` |
...
| `PUT /review/runs/{runId}/bands/{dimension}` | Confirm, override, or set unassessed (FR-181) | Rev | `BandDecision` | `{ band: BandView, run: RunSummary }` | `BAND_DECISION_INVALID`, `RUN_NOT_SCORED` | write | `{"decision":"overridden","band":"developing","note":"A wider net is defensible when information is thin."}` |
...
| `PUT /admin/users/{userId}/platform-role` | Set the account's one role (D-748): `student`, `tassl_scenario_editor`, `instructor` or `admin`; revokes the person's sessions and writes `role.set` | Adm | `{ role }` | `AdminUser` | `ROLE_INVALID` | write | `{"role":"instructor"}` |
| `GET /admin/flags` | Flags, effective provider, the runtime assistant mode, and model spend against the budgets | Adm | — | `{ ai, sampleData, testControls, effectiveLlmProvider, aiMode, assistantMode, llmUsage: { today: { calls, tokens, costUsd }, month: { … }, budgets: { userDaily, globalMonthly } } }` | — | read | — |
| `POST /admin/sentry-test` | Sends one `ops.sentry_test` warning from this deployment and answers `{ eventId, environment, dsnConfigured }` (D-708); the flags screen's "Send a test event to Sentry" button calls it | Adm | — | `{ eventId, environment, dsnConfigured }` | — | write | Replaces the operator-side `scripts/sentry-test.ts` for the launch checklist's row 7 |
| `PUT /admin/settings/ai-mode` | Sets the runtime assistant switch (D-691): `mode` is `live` or `mock`; writes `app_settings.ai_mode` and an `ai_mode.set` audit row in one transaction; the next model call honours it | Adm | `{ mode }` | the `GET /admin/flags` object | 422 `VALIDATION_ERROR` when `FEATURE_AI` is off (the environment already forces the scripted assistant) | write | `/api/ready` reports the same value as `assistantMode` |
