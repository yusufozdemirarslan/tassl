...
| `PUT /admin/users/{userId}/institution-role` | Make a member of one institution a Student or an Instructor there (D-747): `member.role`, every `section_memberships` row of theirs in that institution, the revocation of their sessions and a `role.set` audit row (`scope: organization`), in one transaction. `AdminUser` carries `memberships: [{ organizationId, organizationName, role }]` for both admin writes and the list | Adm | `{ organizationId, role }` — `role` is `student` or `instructor` | `AdminUser` | `ROLE_INVALID` (another seat, or the actor's own row), 404 `NOT_FOUND` (no such account, or not a member of that institution) | write | `{"organizationId":"…","role":"student"}` |
