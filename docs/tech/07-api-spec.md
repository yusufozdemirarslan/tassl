...
| `POST /runs/{runId}/addendum` | Add the 50-word addendum (FR-107) | Stu | `{ text }` | `204` | `ADDENDUM_EXISTS`, `ILLEGAL_TRANSITION` (before the lock, or once the run is recorded or voided, D-295) | write | — |
...
| `GET /runs/{runId}/turn` | Turn message and window (FR-110) | Stu | — | `TurnView` | `TURN_NOT_OPEN` (details `{ state }`) | read | `→ {"run":{…},"text":"…","voice":"stakeholder_message","windowEndsAt":"…","remainingMs":719400,"frozen":{"frame":{…},"brief":{…}},"namedFields":[…]}` |
