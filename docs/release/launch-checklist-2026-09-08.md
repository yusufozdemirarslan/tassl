...
| 5 | Restore verified by a drill | partial | `scripts/restore-drill.sh` had never run either: it writes `GITHUB_STEP_SUMMARY` and checks in as a Sentry cron monitor, so it was written for Actions, but no workflow called it and its stated prerequisite (PostgreSQL 17 client tools) is not installed on the builder's machine. `restore-drill.yml` now runs it Monday 06:00 UTC and on dispatch (D-681). Two runs so far, both stopped short, each on a defect the drill itself found. The first: `INVALID_SERVER_ENV` — the workflow carried none of the non-secret defaults `next build` validates. The second got much further and failed at `neon connection-string` with *"Multiple roles found for the branch: neondb_owner, tassl_app"* — Step 15.1's least-privileged role, added the same day, had made the command ambiguous (D-690). What that run *did* prove is the half nobody sees until something goes wrong: the nightly artifact's checksum verified, the throwaway branch was created, and on failure the trap deleted that branch and only that branch (D-584), then checked in to Sentry — which is how the `restore-drill` cron monitor came to exist. A full green drill still has to be observed. |
| 6 | Alerts on | partial | Sentry project `tassl` exists in org `hewot-llc` and the DSN is set in production and preview. The **three cron monitors create themselves from their first check-in** (13 §7), so `nightly-backup`, `restore-drill` and `jobs-drain-daily` need no clicking. The other 15 rules are not created. The token supplied is an organisation **CI** token: `GET /organizations/hewot-llc/releases/` returns 200 while `GET /organizations/hewot-llc/` and every project and alert-rule endpoint returns 403, so its scope is `org:ci` — exactly enough to upload releases and source maps, and nothing else. Creating alert rules needs `alerts:write`, which means either a second token or the Sentry UI in a browser. |
| 7 | Sentry receiving events | pass | `NEXT_PUBLIC_SENTRY_DSN=... APP_ENV=production pnpm exec tsx scripts/sentry-test.ts` → event `591d743baa384ab88ad77b84008330c6`, which arrived in Sentry → Issues as `ops.sentry_test`, level Warning, short id `TASSL-1`, within a minute. |
| 8 | Sentry release tagging | pass | Release `e08a71a477c0fb21a273c340bd0f8b9406a4d081` exists under project `tassl` (`GET /organizations/hewot-llc/releases/`), created by the deploy at 05:47. Source maps uploaded in the build step — the log lists each bundle with its `debug id`. The legacy release-`files` collection reads 0, which is expected: modern `@sentry/nextjs` uploads artifact *bundles* keyed by debug id rather than files attached to a release. The DSN is inlined in the deployed client bundle (`/_next/static/chunks/2d3ei0-gpmh2g.js`). |
| 9 | PostHog receiving events | partial | The pipeline is wired end to end and verified from the outside: the project key is inlined in the deployed client bundle (`/_next/static/chunks/2d3ei0-gpmh2g.js`), the `/ingest/*` rewrite reaches PostHog (an empty POST returns PostHog's own `400`, not a Next.js 404), and a real sign-in returns 200, which fires `sign_in_succeeded` server-side through `posthog-node`. What is **not** verified is the last hop — an event visible in PostHog → Activity. That needs either the PostHog UI or a *personal* API key; the value supplied is a project write key (`phc_…`), which cannot read events back. |
...
| 12 | Production env vars set | pass | Present: `APP_ENV`, `BETTER_AUTH_SECRET`, `CRON_SECRET`, `DATABASE_URL`, `DATABASE_URL_UNPOOLED`, `EMAIL_FROM`, `EMAIL_TRANSPORT`, `FEATURE_AI`, `LLM_API_KEY`, `LLM_BASE_URL`, `LLM_PROVIDER`, `NEXT_PUBLIC_APP_URL`, `SEED_PASSWORD`, `TASSL_APP_DB_PASSWORD`. Correctly absent: `FEATURE_TEST_CONTROLS` (D-023 — the default `true` is what walkthrough step 7 needs). `NEXT_PUBLIC_SENTRY_DSN` and `NEXT_PUBLIC_POSTHOG_KEY` are now set in production and preview. `RESEND_API_KEY` stays empty and `EMAIL_TRANSPORT` is explicitly `console` (D-686): **production logs email and delivers none**, because there is no sending domain to verify. Seeded seats are unaffected; self-service sign-up cannot complete until a domain exists. |
...
| 16 | Cross-browser E2E | pass | `cross-browser.yml` run 34320306316: **firefox 9m34s success, webkit 10m54s success**, `fail-fast: false` so neither hid the other. Chromium is green on every PR through `checks.yml`. All three engines pass (NFR-010). The row had been unreachable as written — it assumed a local run, which needs a local Postgres, and this machine has no Docker (D-687). |
| 17 | Walkthrough script | not run | Step 15.5. The automated equivalent (`tests/e2e/walkthrough`) is green in CI on every run. |
