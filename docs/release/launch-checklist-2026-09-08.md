...
| 16 | Cross-browser E2E | pending | The row assumed a local run, which needs a local Postgres, and this machine has no Docker — so it could not be closed by anyone with the tooling present. `cross-browser.yml` now runs firefox and webkit on dispatch and every Monday (D-687), `fail-fast: false` so one engine's failure does not hide the other's. Awaiting its first run. |
