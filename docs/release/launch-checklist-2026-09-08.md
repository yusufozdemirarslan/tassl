...
| 6 | Alerts on | partial | Sentry project `tassl` exists in org `hewot-llc` and the DSN is set in production and preview. The **three cron monitors create themselves from their first check-in** (13 §7), so `nightly-backup`, `restore-drill` and `jobs-drain-daily` need no clicking. The other 15 rules are not created: the auth token the builder issued is an organisation CI token, which carries `org:ci` only — it uploads releases and source maps but returns 403 on every project and alert endpoint. Creating them needs a token with `alerts:write` and `project:write`, or the browser. |
...
| 8 | Sentry release tagging | pending | `SENTRY_AUTH_TOKEN` is a GitHub secret and `SENTRY_ORG` (`hewot-llc`) a GitHub variable; `SENTRY_PROJECT` is `tassl` in the workflow and that project exists, confirmed against the DSN's own project id. Verify on the next production run: Sentry → Releases → the `main` head SHA, Artifacts tab non-empty. |
| 9 | PostHog receiving events | pending | Project `600786`; `NEXT_PUBLIC_POSTHOG_KEY` set in production and preview. It is a `NEXT_PUBLIC_` value, so it is inlined at build — the redeploy that carries it is what makes client events fire. Verify after it lands by signing in and watching PostHog → Activity. |
