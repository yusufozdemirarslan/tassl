...
**The Underlined-In-Prose Rule.** A link _inside a sentence_ is underlined at rest. Teal on muted ink is a colour difference of 1.26:1, so colour alone does not tell a reader where a link starts or ends (WCAG 1.4.1). A link that _stands on its own_ — its own row, its own list item, its own line — has nothing around it to be confused with, so it may take the underline on hover and carries the 40 px target instead (D-624).
...
- Breakpoints: `sm` 640, `md` 768, `lg` 1024, `xl` 1280, `2xl` 1536. The app shell is a 56 px header, a 224 px left rail, and `main` at `md` and above; under `md` the rail becomes a fixed bottom bar and `main` keeps 80 px clear beneath its content for it. Anything else fixed to the bottom edge — the toast region is the only one — clears that bar by the same 80 px (D-625).
- **`main` stops growing at 96 rem** (`max-w-page`, `--container-page`), centred in whatever is left beside the rail. The prose was already clamped by the measure; the widths that ran away were the ones a measure does not govern — the replay's trace, the roster, the claims table, the confirmation workspace — which on a 2560 px monitor ran 2288 px edge to edge. 96 rem is the `2xl` breakpoint, which is the widest arrangement this product has anything to put in, and it is in `rem` so it scales with browser text zoom the way the measure does (D-620).
- **The writing screens split at `2xl` and at no width below it.** A screen a student writes on is one column until the width at which a two-column split can still hold the full reading measure in the writing column, and two columns from there. `2xl` (1536) is that width: `main` is then 1264 px, and a 5/6 split leaves the writing column about 677 px, which is the ~650 px a 60ch measure needs plus its 24 px panel padding. Below it the writing runs full width and the measure clamps it. The rule exists because the alternative was measured and is worse: the run workspace's three columns at `lg` gave the Decision Brief 216 px and about twenty characters a line, and dragging the window from 1023 to 1024 made the editor _narrower_. A layout that shrinks the writing surface as the window grows is a bug, not a breakpoint (D-310). The **confirmation workspace** answers the same question with its own arithmetic and its own breakpoint: its left track is a fixed navigator rather than a second content column, so it splits at `xl` against an 18 rem tree, which leaves the element editor 696 px — the first arrangement that clears the ~682 px its measure and padding need. It had been splitting at `lg` against a 20 rem tree, where one pixel of window cost the editor 344 of them (D-628).
- **The Monotonic-Width Rule.** No element is narrower at a wider viewport than at a narrower one. A `max-w` that appears at a breakpoint and is smaller than the width the layout already gave the element below that breakpoint is the same bug as the split above, in one class: the header's institution name was uncapped under `md`, clamped to 16 ch at `md`, and did not recover until `lg`. A cap exists to stop an element spreading where there is room, so it is the same cap wherever there is room for it, and `min-w-0 truncate` inside a shrinking row handles the widths where there is not (D-627).
...
- Density: matrix and timeline at 12/18 Mono with 8 px cells; writing surfaces at 16/26 with 24 px padding. A dense **table** that scrolls sideways keeps a sticky first column, and that column is capped at a width measured against the narrowest viewport rather than guessed — the claims table's 15 rem left 56 px of a 296 px scroll region, which is reachable but not readable (D-630).
- **A drawing is never scaled below 1:1.** The stance matrix is the case: its plot _is_ text, so `preserveAspectRatio` fitting it to a 296 px box drew it at 0.61 and set its labels at seven pixels. Below its own width the figure keeps its size and its region scrolls sideways; above it, it grows to 1.25× and stops, because past that a five-by-five grid of counts is a poster. A reserved height belongs to a chart that arrives in a later chunk; a server-rendered one takes `height="auto"` and states its own size (D-621).
...
The stored JSON of one event, opened in place: a native `<details>` whose summary is a text link and whose body is a `--paper-sunken` `<pre>` in Mono dense, wrapping and scrolling sideways for what will not fold. Two screens show it — the replay's trace and the admin audit log — and both take it from one component (`RecordDisclosure`), which is a Server Component, has no message catalogue of its own, and is handed its words. `<details>` and not a dialog: it opens with the keyboard, opens to find-in-page, prints open, and costs the route nothing (D-623).
...
- **Every floating surface is bounded by the viewport, in the primitive rather than at the call site.** A dialog is `fixed` and centred with `-translate-y-1/2`, so a tall one does not overflow a parent — it overflows the _viewport_, off both ends at once, with nothing to scroll and its own confirm button out of reach. `DialogContent` and `AlertDialogContent` carry `max-h-[calc(100dvh-2rem)] overflow-y-auto`, the vertical twin of the `max-w-[calc(100%-2rem)]` beside them; `SheetContent` carries `overflow-y-auto`; every popup carries a width clamp against `100vw`. Five of twenty-four dialogs had written the height guard by hand and nineteen had not, which is what makes it the primitive's (D-622).
- **A title never runs under the close button.** The close control is absolutely positioned in the corner, so `DialogTitle` and `SheetTitle` reserve 32 px of right padding; at 360 px a wrapped title overlapped it by 24 to 28 px. The control itself is 32 px of visual with `after:-inset-1` carrying it to the 40 px target (D-626).
...
- **Toast:** Plex Sans on raised paper with `--shadow-float`, title in ink, description in `--ink-muted`; the region is labelled "Messages". It sits bottom-right, and under 600 px it goes full width 80 px from the bottom edge rather than 16 — a toast is pointer-interactive and the fixed rail is down there, so at the library default every message covered the whole of primary navigation and swallowed the taps aimed at it (D-625).
